...
export default function FavouritesCrud() {
  ...
  const peticionPost=async()=>{
    await axios.post(url, favouriteSelected)
    .then(response=>{
  ...
  const peticionPut=async()=>{
    await axios.put(url+"/"+favouriteSelected.id, favouriteSelected)
    .then(response=>{
  ...
  const peticionDelete=async()=>{
    await axios.delete(url+"/"+favouriteSelected.id)
    .then(response=>{
      setFavourites(favourites.filter(favourite=>favourite.id!==favouriteSelected.id));
...
<TextField className={styles.inputMaterial} label="Imagen" name="image" onChange={handleChange} value={favouriteSelected&&favouriteSelected.image || ""}/>
  ...
  return (
      ...
      <><CrudImageList images={favourites} abrirCerrarModalInsertar={abrirCerrarModalInsertar} selectImage={selectImage}/><Modal
      open={modalInsertar}
