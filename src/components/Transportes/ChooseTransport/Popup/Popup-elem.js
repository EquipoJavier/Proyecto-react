...
export default function PopupElem(props) {
    ...
    return (
        <>
        {/* <img className="popup__img--thumbnail" src={transport.img} alt={transport.name} /> */}

        <span className="popup__img--name" onClick={function() { // *2
            setButtonPopup(true)
        }}>{transport.name}</span>

        <HiddenBox transport={transport} buttonPopup={buttonPopup} setButtonPopup={setButtonPopup}/> {/* *3 */}
        </>
    )
}